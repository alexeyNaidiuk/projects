...
class ConcreteSpam(Spam):
    ...
    def try_to_post_response(self, s: requests.Response, target: str, text: str, token: str) -> requests.Response:
        post_response = None
        while post_response is None:
            try:
                post_response: requests.Response | None = self.post(s=s, target=target, text=text, token=token)
            except Exception as error:
                print(error)
    ...
    def try_to_post(self, target, text) -> requests.Response:
        ...
        post_response = self.try_to_post_response(s=s, target=target, text=text, token=token)
        return post_response
    ...
    def get(self, s: requests.Session) -> requests.Response:
        ...
        response = s.get(url, headers=headers, verify=False, proxies=self.get_proxies())
        return response

    def post(self, s: requests.Session, text, target, token) -> requests.Response:
        ...
        response = s.post(url, headers=headers, data=data, verify=False, proxies=self.get_proxies())
        return response
